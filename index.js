...
app.get("/", (req, res) => res.send("Signal server listening"));
...
io.on("connection", (socket) => {
  ...
  socket.on("register-email", (email) => {
    ...
    if (client) {
      if (client.email && client.email !== email) io.to(socket.id).emit("disconnect-call");
      client.email = email;
  ...
  socket.on("initiate-call", ({ targetId, offer }) => {
    ...
    if (target) io.to(targetId).emit("offer", { callerId: socket.id, offer });
  });

  socket.on("answer", ({ answer, targetUserId }) => {
    const target = clients.find(c => c.socketId === targetUserId);
    if (target) io.to(targetUserId).emit("answer", { answer, calleeId: socket.id });
  });

  socket.on("ice-candidate", ({ candidate, targetUserId }) => {
    const target = clients.find(c => c.socketId === targetUserId);
    if (target) io.to(targetUserId).emit("ice-candidate", { candidate, from: socket.id });
  });

  socket.on("decline-call", ({ targetUserId }) => {
    const target = clients.find(c => c.socketId === targetUserId);
    if (target) io.to(targetUserId).emit("disconnect-call");
  });
